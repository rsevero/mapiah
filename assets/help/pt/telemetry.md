...
Como em todo software de código aberto, você pode revisar o código-fonte para ver exatamente como a telemetria funciona.

O código relevante está em `src/telemetry/` e `src/controllers/MPTelemetryController.cpp`.

Os dados de telemetria são armazenados localmente no arquivo de configurações do Mapiah, e enviados ao servidor via solicitações HTTPS POST para `https://api.mapiah.org/telemetry`.
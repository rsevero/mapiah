...
Unlike Phases 1-6, Phase 7 does **not** start from nothing: Mapiah already has a complete, pre-project-feature "Run Therion" subsystem — `MPTherionRunner` (`lib/src/auxiliary/mp_therion_runner.dart`), platform-specific runners, and `MPRunTherionDialogWidget` — with buttons, an overflow-menu entry, and keyboard shortcuts (`T` / `Ctrl+T`) already wired into the then-separate `MapiahHome` and `TH2FileTabsPage` pages. Phase 7 closes the two gaps the roadmap's phase table names, while also **removing** the piece of state (`MPGeneralController.thConfigFilePath`) whose sole purpose was standing in for the project-tree's own `rootConfigPath`:
...
2. **Merge "Choose THConfig file and run Therion" into "Open project and run Therion"**: the empty-project-state button and shortcut (`Ctrl/Cmd+T`) that used to pick a standalone `thconfig` (via a plain file picker, entirely disconnected from the project tree) now pick a **project** file (the same picker `MPDialogAux.pickProjectFile` already uses) and, once picked: (a) start the Therion run immediately — same timing as today, so the dialog appears and compilation begins with no perceptible delay — and (b) *without waiting for the run to finish*, kick off `THProjectController.openProject` in the background so the project tree populates concurrently with the compile. Both entry points are available only while no project is loaded; once a project is loaded, the UI exposes only the plain "Rerun Therion" action (`T`) for that project's root config.
3. **"Rerun Therion" (`T`) now runs `THProjectController.rootConfigPath`** and is enabled exactly when a project is loaded (`rootConfigPath.isNotEmpty`), replacing the old `MPGeneralController.thConfigFilePath.isNotEmpty` enablement check in both the `MapiahHome` and `TH2FileTabsPage` pages.
...
- **No new dialog, no new keyboard shortcut, no new icon.** The existing `Ctrl/Cmd+T` shortcut and `Icons.playlist_add_check_outlined` button are repurposed for "Open project and run Therion" in the empty-project state. Once a project is loaded, neither that button nor `Ctrl/Cmd+T` is available; the existing `T`/`Icons.play_arrow_outlined` entry point reruns Therion against the loaded project's root config.
...
- **`MPGeneralController.thConfigFilePath` has exactly one reason to exist today, and it is the one this phase removes.** A repo-wide grep for `thConfigFilePath` (excluding generated `.g.dart`/localization files) shows it used only as: (a) the value threaded into `MPRunTherionDialogWidget(thConfigFilePath: ...)` from `MPDialogAux.runTherion`, and (b) an `.isNotEmpty` gate that enables the "Run Therion" button/menu-entry in `MapiahHome` (lines 205, 278) and `TH2FileTabsPage` (lines 243, 433). It is never displayed, never persisted, and never read by anything else — it is purely a stand-in for "which `thconfig` should Rerun Therion use," which `THProjectController.rootConfigPath` already is for the project-tree feature.
- **Three call sites currently invoke the pick-and-run / rerun pair**, all needing their target renamed/updated: `MapiahHome`'s expanded-app-bar buttons (lines ~184-222) and its compact overflow menu (`_MapiahHomeAction.openTHConfig`/`.runTherion`, lines ~312-320, ~363-366); `TH2FileTabsPage`'s equivalent buttons (lines ~216-260), compact menu, and `_withShortcuts` block (~872-879); and the canvas-editing state machine's own key-down handling (`mp_th2_file_edit_state_key_down_mixin.dart:193,197` → `mp_th2_file_edit_state.dart:285-286,358-359`, via `MPButtonType.chooseTHConfigAndRunTherion`/`.runTherion`) for when a `.th2` canvas (not the tab bar) has keyboard focus. All three currently call `MPDialogAux.chooseTHConfigAndRunTherion`/`runTherionWithLastTHConfig`.
- **`MPDialogAux`'s current pick-and-run chain** (`lib/src/auxiliary/mp_dialog_aux.dart:1214-1486`): `pickTHConfigFile` (a generic file picker, `MPFilePickerType.thconfig`, sets `MPGeneralController.thConfigFilePath`) → `pickTHConfigFileAndRunTherion` (calls `pickTHConfigFile` then `runTherion`) → `chooseTHConfigAndRunTherion` (Therion-availability gate, then `pickTHConfigFileAndRunTherion`) → `runTherion` (reads `MPGeneralController.thConfigFilePath`, shows `MPRunTherionDialogWidget`, and — on the dialog popping `true`, meaning the user pressed `Ctrl+T` *inside* the dialog — recurses into `chooseTHConfigAndRunTherion` again). `runTherionWithLastTHConfig` is the separate "rerun" path (Therion-availability gate, then `runTherion` with no picker). `runTherionWithTHConfigFile(context, path)` (used by CLI startup in `MapiahHome`, lines 101 and 129) sets `MPGeneralController.thConfigFilePath` directly (bypassing the picker) then calls `runTherion`.
- **`pickProjectFile`** (`mp_dialog_aux.dart:1266-1317`, `MPFilePickerType.project`) is the picker the merged flow reuses: same `FilePicker.pickFile` shape/dialog title/`lastAccessedDirectory` bookkeeping as `pickTHConfigFile`, but it `await`s `THProjectController.openProject(pickedFilePath, forceConfigShape: true)` before optionally calling `ensureProjectTabsPageOpen(context)`. Today it already has several call sites — the `MapiahHome` app-bar "Open Project" button (line 176, `Icons.folder_open_outlined`), its overflow-menu equivalent (line 362), its `Ctrl+O`/`Cmd+O`/Shift variants in `_withShortcuts` (~lines 510-525), and the project tree's own empty-state "Open Project" button (`th_project_tree_widget.dart:162`) — none of them touched by Phase 7; `pickProjectFile` stays exactly as the "load a project without also running Therion" entry point everywhere it's already wired in.
- **The run dialog has its own internal `Ctrl+T` rebind before this phase.** `MPRunTherionDialogWidget` (`mp_therion_run_dialog_widget.dart:376-402`) binds `T` → rerun (`_RerunTherionIntent`, calls `_rerunTherion()` only if not already running) and `Ctrl+T` → `_ChooseTHConfigAndRunTherionIntent`, whose handler stops the current run and pops the dialog with `Navigator.of(context).pop(true)`. Phase 7 removes that choose-another-config intent and the corresponding boolean dialog result: by the time this dialog is open, its target is the loaded project, so `Ctrl/Cmd+T` must be unavailable and plain `T` remains the only Therion shortcut inside the dialog.
...
lib/src/
 ...
 ├── pages/
 │    └── th2_file_tabs_page.dart                    # Existing: button/menu wording + enablement retargeted
 ...
 └── widgets/
      ├── mp_therion_run_dialog_widget.dart           # Existing: removes in-dialog Ctrl/Cmd+T; calls the new diagnostics bridge once a run finishes
...
The dialog no longer returns a "pick a different project" result: once a project is loaded, changing projects is an explicit close/open workflow. Apart from that removal, this is behaviorally identical to the old `runTherion`, minus the source of `thConfigFilePath` (now a parameter instead of `mpLocator.mpGeneralController.thConfigFilePath`).
...
For the §4.3 picker flow, Therion availability was already confirmed `true` by `pickProjectFileAndRunTherion`'s own top-of-function gate before the picker even opened, so this helper's `isTherionAvailable` check is redundant-but-harmless there; for the CLI-argument flow (no picker, no prior gate) it is the only place that check happens, exactly replacing what `runTherionWithTHConfigFile` did today. `MapiahHome._runStartupFileActions` keeps using `await`, now as `await MPDialogAux.runTherionAndOpenProjectInBackground(context, widget.thConfigFilePath!)` (and the positional-argument equivalent). The helper returns the `runTherion` future, so CLI startup still waits for the Therion run dialog to close before the post-startup telemetry-consent/update-check block runs — exactly the existing `runTherionWithTHConfigFile` sequencing — while `openProject` still proceeds in the background.

### 4.5 Button/menu/shortcut call-site updates

- In the project tree's empty state, the `Icons.playlist_add_check_outlined` button calls `MPDialogAux.pickProjectFileAndRunTherion(context)` and is labeled "Run Therion (and open project)" (with the equivalent Portuguese localization).
- `Ctrl/Cmd+T` invokes `pickProjectFileAndRunTherion` only while no project is loaded. It is intentionally unavailable once `THProjectController.rootConfigPath.isNotEmpty`; switching projects is then an explicit close/open workflow rather than an implicit side effect of a run command.
- The `Icons.play_arrow_outlined` app-bar button and compact overflow-menu entry are enabled exactly when `rootConfigPath.isNotEmpty` and invoke `MPDialogAux.rerunTherionForOpenProject(context)`. Since `rootConfigPath` is `@observable`, their surrounding `Observer` picks up project open/close automatically.
- The plain `T` shortcut invokes `rerunTherionForOpenProject` and naturally no-ops while no project is loaded.
- `MPRunTherionDialogWidget` does not offer an in-dialog choose-another-project shortcut once its run targets a loaded project; rerunning that same project remains available through plain `T`.
...
Step 3: Remove MPGeneralController.thConfigFilePath / setTHConfigFilePath;
        ...
        let the already-running build_runner watch regenerate
        mp_general_controller.g.dart (do not run `dart run build_runner
        build` manually)
   │
   ▼
Step 4: Update call sites: project-tree empty-state action,
        TH2FileTabsPage rerun button/compact menu/_withShortcuts, the canvas
        key-down handling, and MPRunTherionDialogWidget. Gate Ctrl/Cmd+T on
        there being no loaded project; remove the orphaned
        mapiahTherionSelectTHConfigDialogTitle .arb key and run
        `flutter gen-l10n` to regenerate the localization files
   │
   ▼
...
2. **"Rerun Therion" targets whatever project is currently loaded**, with no picker step, and is disabled with no project loaded. Conversely, the "open project and run" button and `Ctrl/Cmd+T` are available only in the empty-project state.
...
- The project-tree empty-state action uses `projectTreeRunTherionButton` ("Run Therion (and open project)" and its Portuguese equivalent). No loaded-project toolbar/menu label for "Open project and run Therion" is needed because that action is intentionally unavailable after a project is loaded.

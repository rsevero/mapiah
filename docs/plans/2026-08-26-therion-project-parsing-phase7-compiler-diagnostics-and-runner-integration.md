...
In both the `MapiahHome` and `TH2FileTabsPage` pages:
...
Step 3: Remove MPGeneralController.thConfigFilePath / setTHConfigFilePath;
   ...
   │
   ▼
Step 4: Update call sites: MapiahHome, TH2FileTabsPage (buttons +
        ...
        in-dialog Ctrl+T handler, MapiahHome._runStartupFileActions;
...
2. **"Rerun Therion" targets whatever project is currently loaded**, with no picker step, and is disabled with no project loaded — verified across both the `MapiahHome` and `TH2FileTabsPage` pages.

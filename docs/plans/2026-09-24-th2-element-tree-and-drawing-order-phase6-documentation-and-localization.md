...
The sidebar row and tab should continue to indicate broken status, and the current Save/Save As/Save All protections remain untouched. If a load throws rather than returning `TH2FileProblem`s, keep its existing load-failure handling and Reload action; the eight category messages apply only to parsed problems.
...
Review both `assets/help/{en,pt}/keyboard_shortcuts_edit.md` tables against the actual state-machine bindings. The bindings are `Ctrl+]` (forward), `Ctrl+[` (backward), `Ctrl+Shift+]` (front), and `Ctrl+Shift+[` (back), with Meta accepted on macOS. Keep all four rows once each, format them like neighboring rows, and place them alphabetically by their **Function/Ação** label. In the editor help, explain that the shortcuts act on selected drawing elements in the active canvas tab; scraps use tree controls. Check the existing Ctrl/Shift-click tree-selection rows for the same formatting and order. This is a documentation correction, not a shortcut change.

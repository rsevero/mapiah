...
- Replace `String? reasonKey` with `MPHierarchyMoveRejection? rejection`, a new enum in the same file:
  ...
  | `duplicateElement` | new, checked for grouped moves | generic |
  | `invalidGroupPlacement` | new, checked for grouped moves | generic |
...
- Add `TH2HierarchyAux.resolveMoveGroups(th2File, parentMPID:, groups:)`. Each `MPMoveGroup` has `elementMPIDs`, an `MPMoveGroupPlacement` (`beforeSibling`, `afterSibling` or `end`) and a nullable `siblingMPID`. Assert that `beforeSibling` and `afterSibling` have a sibling and `end` has none. `end` inserts before the scrap's `THEndscrap`, or after the last scrap at file level. This is a real placement, not a null before-anchor: `resolveMoves` maps its existing `beforeSiblingMPID == null` request to a single `end` group, so end-of-scrap and end-of-file requests can delegate to the shared resolver. The resolver copies the affected parents' **full** `childrenMPIDs` lists once, then resolves each group against those same simulated lists, removing and inserting each element before calculating the next move's index. It never mutates `th2File`. Do not call the public `resolveMoves(th2File, ...)` separately for each group: each call would start from the original order. `MPMoveElementsCommand._prepareUndoRedoInfo` simulates the already resolved moves to record inverse positions; it does not resolve the forward indices.
- Add a pure `TH2HierarchyAux.validateMoveGroups(th2File, parentMPID:, groups:) → MPHierarchyMoveCheck`, shared by the order preview and `MPCommandFactory.moveElementGroups`. An empty group list passes validation and resolves to a no-op; a group with no elements is rejected as `emptySelection`. The validator rejects a missing sibling for `beforeSibling`/`afterSibling` or a sibling supplied with `end` as `invalidGroupPlacement`, an MPID repeated within or across groups as `duplicateElement`, and any before/after anchor in the union of all groups' MPIDs as `targetIsMoving`. It then calls `validateMove` for each group with that group's own `elementMPIDs`, the common target parent and its before/after anchor. This preserves the per-group area-border checks while also enforcing the rules that span groups. `MPMoveGroup` also asserts the placement/sibling shape in debug builds, but the validator enforces it in release builds. The factory calls the validator before `resolveMoveGroups`, returns `null` on rejection or no-op, and otherwise emits one `MPMoveElementsCommand`; a direct factory call cannot bypass validation. The preview uses the same validator, so its availability and rejection reason match execution.
- **Anchors are the stepped-over sibling.** Each group is anchored on the non-selected sibling it steps over, which is never an element of the action and so never moves while the action is resolved:
  - **Send backward:** `placement = beforeSibling`, with `siblingMPID` set to the non-selected movable sibling just before the run.
  - **Bring forward:** `placement = afterSibling`, with `siblingMPID` set to the non-selected movable sibling just after the run.
...
- Add a pure `previewDrawingOrderAction(action, elementMPIDs:)` on `TH2FileEditElementEditController`, with an `MPDrawingOrderAction` enum for the four actions. It returns an `MPMoveElementsPreview` containing either a rejection, no-op, or the resolved `List<MPElementMove>` plus the request needed for execution. A shared private planner returns a no-op for an empty selection, then rejects broken files and mixed selections and builds the groups for relative actions or the single boundary request for front/back. It calls `validateMoveGroups` for relative actions and `validateMove` for the boundary request, then calls `resolveMoveGroups` or `resolveMoves` to detect no-ops. It reads the model without making a command, changing selection or pushing undo. The four execution methods use this same planner, then call `MPCommandFactory.moveElementGroups` for relative actions or `moveElements` for boundary actions only when the preview would move something. If any group is rejected, the whole action is rejected and no command is created. This preserves the broken-file check that `_moveRelative` currently gets through `checkMoveElements`.
...
- **Ctrl+click** (Meta on macOS, via `MPInteractionAux.isCtrlPressed()`/`isMetaPressed()`, as the canvas does) on an element row of the **active scrap** toggles that element in the selection. Adding uses `selectionController.addSelectedElement(element, setState: true)`. Removing uses `removeSelectedElementsByMPIDs(<int>[mpID])` followed by `setSelectionState()`, not `removeElementFromSelectedLogical(mpID, setState: true)`: like the canvas deselect path, `removeSelectedElementsByMPIDs` updates the snap targets and the status bar, which `removeElementFromSelectedLogical` does not, and `_setState` only refreshes the status bar when the state actually changes, so deselecting one of several elements would otherwise leave both stale.
- **Stray scrap entries are dropped first.** Before toggling, a Ctrl+click or Shift+click on a point, line or area row removes every `MPSelectedScrap` from `mpSelectedElementsLogical` (the scraps dialog's `setSelectedScrapByMPID` leaves one there indefinitely, §3.6), with `removeSelectedElementsByMPIDs` on their MPIDs. This runs **before** `onButtonPressed(MPButtonType.select)`: otherwise `setSelectionState()` would see a non-empty map holding only a scrap and enter `selectNonEmptySelection` with a scrap-only selection, the state in which canvas drags and arrow keys throw for an `MPSelectedScrap`. The tree therefore never creates a mixed selection.
...
- **Leaving creation mode first.** Ctrl+click and Shift+click on point, line and area rows run the shared tree mode-exit helper (§6.2), which drops stray scrap entries and calls `controller.stateController.onButtonPressed(MPButtonType.select)`, and only then toggle or extend the selection:
  - Plain taps, **every** right-click on an element row (including a selected point, line, area or scrap; §6.2), Ctrl+click on scrap rows (§3.6) and every tree edit (§3.5) leave creation mode this way. Before the mode exit, tree selection clicks also remove stray `MPSelectedScrap` entries as above, so `setSelectionState()` never enters `selectNonEmptySelection` with a scrap-only selection.
...
- For example, `MPCommandFactory.moveElementsFromDeltaOnCanvas`, used by canvas drags and arrow keys in `MPTH2FileEditStateSelectNonEmptySelection`, throws `ArgumentError` for an `MPSelectedScrap`, and `addSelectedElement` does not put a scrap in the map (it still adds its MPID to `_isSelected` and sets the option type to PLA).
- Today a scrap enters that map in two ways. The scrap copy, cut and duplicate helpers put it there through `setSelectedElements` and clear it again right away. The scraps dialog's right-click (`setSelectedScrapByMPID`) puts it there **without** changing state, and nothing clears it when the scrap options window closes: it stays until the next selection change. The tree's modified clicks remove such an entry before anything else (§3.4). A canvas Shift+click in `selectNonEmptySelection` after a dialog right-click can still add a point, line or area next to it, so the mixed-selection rejection (§3.1) remains necessary.
...
- **Model:** `TH2FileEditSelectionController` gets `@observable ObservableSet<int> selectedScrapMPIDs`, with the actions `toggleSelectedScrap(int scrapMPID)` and `clearSelectedScraps()`, and a `selectedScrapMPIDsInFileOrder` getter. The build_runner watch regenerates the `.g.dart` file; do not run it manually. Ids that are no longer scraps of the file (after a scrap removal, its undo or a reload) are pruned by `TH2FileEditController.bumpStructureRevision` (`th2_file_edit_controller.dart`), which calls a new selection-controller action for it; the getter skips them in any case.
- **Never mixed with points, lines and areas:** `addSelectedElement` clears `selectedScrapMPIDs` when the element is a point, line or area, which also covers `addSelectedElements` and the point, line and area branch of `setSelectedElements`, since both go through it. A `setSelectedElements` call that adds only scraps (the scrap copy, cut and duplicate helpers) or nothing leaves `selectedScrapMPIDs` alone, so those helpers do not wipe the tree's scrap selection. A Ctrl+click on a scrap row clears a non-empty canvas selection first. So the scrap selection and a point, line or area selection are never both non-empty. The scraps dialog's `setSelectedScrapByMPID` does not touch `selectedScrapMPIDs`.
- **Ctrl+click** (Meta on macOS) on a scrap row of a loaded, valid file:
  1. Run the shared tree mode-exit helper (§6.2), so stray `MPSelectedScrap` entries are removed and line or area creation ends as for any tree selection;
...
- **Plain click on a scrap row** first runs the shared tree mode-exit helper (§6.2), like a plain click on a point, line or area row, so an unfinished line is finalized (Phase 3's `_onScrapTap` only called `setActiveScrap`, which forces `selectEmptySelection` without that mode exit). It then makes the scrap active, as in Phase 3, and clears the scrap selection. **Shift+click on a scrap row** behaves as a plain click (§2).
...
- **Mixed selections.** A selection can hold a scrap next to points, lines or areas only in edge cases (the scraps dialog selects a scrap with `setSelectedScrapByMPID`, and a canvas Shift+click in `selectNonEmptySelection` can then add an element; the tree's modified clicks remove the scrap first, §3.4). Dragging a row of such a selection still drags the whole selection, and every target rejects it with `mixedScrapsAndDrawables` (§3.1); the drag never silently drops part of it. `isScrap` is `true` only when every dragged element is a scrap; for a mixed payload the zones below are read as for points, lines and areas, which does not matter since every drop is rejected.
...
- **Disabled items:** the menu builder calls the pure `previewDrawingOrderAction` (§3.3) for each order item. A preview with no moves disables the item (`onPressed: null`); a rejected preview also shows its reason in a second line. The builder never calls `bringForward`, `sendBackward`, `bringToFront` or `sendToBack`, since those execute commands. A "Move to scrap" entry that `checkMoveElements` rejects is disabled, and its label is followed by the reason in a second, smaller line. A second line is used instead of a tooltip because it is visible without hovering and also works with keyboard navigation inside the open menu.
...
- Every action runs `prepareTH2FileForTreeEdit` first. It then checks that the menu target still exists and runs the order planner again against the current model, because the file can change while the menu is open. It never executes the menu builder's earlier preview. It applies the §4 selection only when the action returns `executed`; a stale disabled/enabled state in the open menu cannot bypass move validation.
...
- `THProjectTreeRowContextMenuWidget` gets an optional `VoidCallback? onBeforeOpen`, called in `_onSecondaryTapUp` before `menuChildrenBuilder`, so the builder sees the current selection and model and stays free of side effects (it is called twice per right-click: on the tap and again after the next frame). Every point, line, area and scrap row uses this hook; other rows behave as in Phase 3.
- For **every** element-row right-click, including on a selected row, `onBeforeOpen` first removes stray `MPSelectedScrap` entries from `mpSelectedElementsLogical` with `removeSelectedElementsByMPIDs`, then calls `onButtonPressed(MPButtonType.select)`. This exits line or area creation and single-line edit before the menu is built. Finalizing a line can add a tree row, so the following selection check uses the current model. If the target row no longer names an element, no menu opens.
- On a **selected** point, line or area row after that mode exit, the menu keeps the whole remaining selection and acts on it. On an **unselected** row, it makes that element's scrap active and selects only that element before opening the menu. To keep plain taps and right-clicks aligned, split `_applySingleTap` into a shared mode-exit helper (stray-scrap cleanup and `onButtonPressed`) and `_applySingleTapSelection(controller, element)` (`setActiveScrapByChildElement` and `setSelectedElements([element], setState: true)`). A plain tap calls both helpers and then brings an open tab to the front; a right-click calls the mode-exit helper and calls the selection helper only when the row is unselected afterward.
- Right-click **never opens or activates a tab**, not even an open file's tab. The first menu action does that (§3.5); dismissing the menu leaves the tabs as they were.
- On a **selected scrap** row, the mode exit keeps the scrap selection; the menu acts on every selected scrap. On an **unselected scrap** row, it leaves the active scrap and scrap selection alone; the menu acts on that scrap only. A plain scrap tap and Ctrl+click on a scrap use the same mode-exit helper before their §3.6 behavior.
...
1. **Model/aux** (no UI): the `MPHierarchyMoveRejection` enum with element MPIDs (§3.1), `MPMoveGroupPlacement` with explicit `end`, `TH2HierarchyAux.resolveMoves`/`resolveMoveGroups` and the factory refactor (§3.2–§3.3), `MPCommandFactory.moveElementGroups`, the shared order planner, `previewDrawingOrderAction` and the new `_moveRelative` (§3.3). Update `t2462` and `t3941`, and add `t3947`. Everything must stay green before any widget change.
...
| Area | Files |
|---|---|
| Aux | `lib/src/auxiliary/th2_hierarchy_aux.dart` (rejection and placement enums, `MPMoveGroup`, element MPIDs, `resolveMoves`, `validateMoveGroups`, `resolveMoveGroups`), `lib/src/auxiliary/th2_element_tree_aux.dart` (drag payload, `moveRejectionMessage`, drop-request resolution) |
| Commands | `lib/src/commands/factories/mp_command_factory.dart` (`moveElements` uses `resolveMoves`; new `moveElementGroups`) |
| Controllers | `lib/src/controllers/th2_file_edit_element_edit_controller.dart` (`MPMoveElementsResult` with the rejection enum, `MPDrawingOrderAction`, `MPMoveElementsPreview`, shared order planner, new `_moveRelative`), `lib/src/controllers/mp_general_controller.dart` (`prepareTH2FileForTreeEdit`), `lib/src/controllers/th2_file_edit_selection_controller.dart` (`selectedScrapMPIDs`, clearing it when points, lines or areas are selected and in `deselectAllElements`, so Escape clears it; pruning stale ids; its `.g.dart` is regenerated by the watch), `lib/src/controllers/th2_file_edit_controller.dart` (`bumpStructureRevision` prunes stale scrap-selection ids) |
...
- `resolveMoves` maps a null `beforeSiblingMPID` to an explicit `end` group; `resolveMoveGroups` accepts `end` for a scrap and for the file and inserts at the correct boundary. `validateMoveGroups` rejects a missing sibling for `beforeSibling`/`afterSibling` or a sibling supplied with `end` as `invalidGroupPlacement`, including in release builds.
...
- `validateMoveGroups` accepts valid non-adjacent groups and accepts an empty group list, which resolves to a no-op; it rejects an empty group, an MPID repeated within or across groups (`duplicateElement`), and an anchor that belongs to another moving group (`targetIsMoving`). Direct `MPCommandFactory.moveElementGroups` calls with these invalid inputs or a group that fails `validateMove` return `null` without creating a command. The order preview and execution both use the grouped validator for the groups they plan.
...
- For each of the four order actions, `previewDrawingOrderAction` reports the same rejection or no-op as execution, or the same resolved moves when executable, without changing `childrenMPIDs`, selection or undo history. A preview on a broken file or mixed selection is rejected without building a command.
...
- Ctrl+click deselecting one of several selected elements updates the snap targets and the status bar.
- After a scraps-dialog right-click (`setSelectedScrapByMPID`), a Ctrl+click and a Shift+click on a point, line or area row remove the `MPSelectedScrap` before the mode exit: the editor never enters `selectNonEmptySelection` with a scrap-only selection, and the result holds only points, lines or areas.
...
- Right-click on an unselected PLA row selects it before the menu items are built (a no-op item for the newly selected element is disabled) and does not open or activate a tab, including when the file has an open tab that is not the active one; on a selected row it keeps the multi-selection after leaving edit mode.
- Right-click on a selected PLA row while in single-line edit exits that mode before previewing the menu, keeping the selected line. Right-click on a PLA or scrap row during line creation finalizes the unfinished line before previewing; a removed target opens no menu. A stray `MPSelectedScrap` from the scraps dialog is removed before the mode exit, so right-click never enters `selectNonEmptySelection` with a scrap-only selection.
- Opening a menu previews all four order actions without changing the file or undo stack; its disabled and enabled items match the pure previews.
- Each order item moves the menu target and is one undo step; items that would be no-ops are disabled. Right-click cleanup removes a stray `MPSelectedScrap` from a mixed canvas selection before building the menu; mixed-selection rejection remains covered by the pure preview and shortcut tests.
...
- A menu built before another model change resolves its action again against the current model; a no-op or rejected result does not apply the §4 post-action selection.

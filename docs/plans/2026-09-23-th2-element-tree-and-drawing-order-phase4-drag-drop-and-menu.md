...
- **Leaving creation mode first.** Ctrl+click and Shift+click on point, line and area rows, after dropping stray scrap entries, call `controller.stateController.onButtonPressed(MPButtonType.select)`, exactly as the plain tap does (Phase 3 plan §8), and only then toggle or extend the selection:
...
- Every action runs `prepareTH2FileForTreeEdit` first. It then checks that the menu target still exists and runs the order planner again against the current model, because leaving creation mode can change the file after the menu was built. It never executes the menu builder's earlier preview. It applies the §4 selection only when the action returns `executed`; a stale disabled/enabled state in the open menu cannot bypass move validation.
...
| Area | Files |
|---|---|
| Aux | `lib/src/auxiliary/th2_hierarchy_aux.dart` (rejection and placement enums, `MPMoveGroup`, element MPIDs, `resolveMoves`, `resolveMoveGroups`), `lib/src/auxiliary/th2_element_tree_aux.dart` (drag payload, `moveRejectionMessage`, drop-request resolution) |
...
- `resolveMoves` maps a null `beforeSiblingMPID` to an explicit `end` group; `resolveMoveGroups` accepts `end` for a scrap and for the file, inserts at the correct boundary, and rejects a missing sibling for `beforeSibling`/`afterSibling` or a sibling supplied with `end` (assert).

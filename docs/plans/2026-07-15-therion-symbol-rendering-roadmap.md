...
- [ ] Add `THLabelAlignment` enum (9 values: center, top, bottom, left, right, upperLeft, upperRight, lowerLeft, lowerRight)
- [ ] Add `THLabelSize` enum (tiny, small, normal, large, huge)
- [ ] Implement `drawTherionLabel()` utility in a new `MPLabelPainter` helper
- [ ] Implement white background `RRect` fill with bbox margin
- [ ] Implement `horiz_labels` rotation reset in `MPLabelPainter`
- [ ] Implement `process_uplabel` / `process_downlabel` / `process_updownlabel` containers as `Path` builders
- [ ] Plumb text content from `THPoint` options (`extra`, station ID, altitude value, etc.) through to the painter
- [ ] Wire `MPLabelPainter` into `THPointPainter` as an alternative rendering path for text-type points
- [ ] Update `MPVisualController` entries for all text-based point types to trigger label rendering instead of shape rendering
- [ ] Add label rendering to the Phase 0 visual test harness

...
- The latest prefix in the current tree is `t3919`, while prerequisite Phase 8.5 reserves `t3920`–`t3923`; Phase 9 therefore begins at `t3924`, after confirming the number again at implementation time. Note the repo already contains a `t3918` collision (two files share that prefix), so the confirmation step must scan for duplicates, not just take the maximum.

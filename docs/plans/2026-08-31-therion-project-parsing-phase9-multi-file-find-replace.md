...
**Prerequisite:** The project-controller lifecycle/save-path refactor (project epoch, atomic revision allocator, `THTextFileSaveResult`, parser override map, in-memory full-project reparse, two-layer flush) is now split out as [Phase 8.5](2026-09-01-therion-project-parsing-phase8_5-epoch-revision-save-result-refactor.md) and must land first. Phase 9 **consumes** those contracts; the descriptions in §6.4 and §7.2–7.3 below are retained as the consuming context, but the implementation and focused infrastructure tests belong to Phase 8.5. Phase 8.5 reserves `t3920`–`t3923`; Phase 9 starts at `t3924` and tests only its matcher, navigation, search, widget, replacement-pipeline, shortcut, and documentation behavior.
...
- `THTextEditorController` has **no revision concept or durable load state today**. `loadFile(String filePath)` takes only a path, reads `fileContentsCache` then falls back to `THProjectParser.readFileContent()`, and unconditionally sets `isDirty = false` even when the cached text is project-owned pending content. Its transient `isLoading` flag cannot distinguish a controller that has never loaded from one whose load failed. `setContent(String newContent)` takes only content, `save()` returns `Future<void>` and infers success from `dirtyFilePaths.contains(...)`, and the constructor accepts an optional `THProjectController` for injection. Phase 9 adds the explicit load-state contract in §6.2 and consumes the revision, dirty-snapshot, and project-epoch plumbing delivered by Phase 8.5; the current signatures are the baseline being changed, not existing behavior to reuse.
- `THProjectParser.loadProject()`/`loadFileNode()` are `static` and synchronous (return `THProjectLoadResult`); `THProjectController` runs them inside `Future<THProjectLoadResult>(() => ...)`. Phase 8.5 extends these methods with the override-map parameter described in §7.3; Phase 9 consumes that post-prerequisite contract.
...
- The latest prefix in the current tree is `t3919`, while prerequisite Phase 8.5 reserves `t3920`–`t3923`; Phase 9 therefore begins at `t3924`, after confirming the number again at implementation time. Note the repo already contains real prefix collisions (`t3172`, `t3760`, and `t3918` each have two files), so the confirmation step must scan for duplicates across the whole `test/` tree, not just take the maximum.
...
lib/src/
 ...
 │    └── th_text_editor_controller.dart          # Shared matcher + flush/reveal/load-state additions
 ├── elements/th_project_search/          # NEW in Phase 9
 │    ├── th_project_search_scope.dart
 │    ├── th_project_search_match.dart
 │    ├── th_project_search_file_result.dart
 │    └── th_project_search_failure.dart
 ├── elements/th_project/                 # DELIVERED BY PHASE 8.5 — Phase 9 only consumes these
...
`elements/th_project/th_project_reparse_result.dart` and `elements/th_project/th_text_file_save_result.dart` are **created by [Phase 8.5](2026-09-01-therion-project-parsing-phase8_5-epoch-revision-save-result-refactor.md)**, not by Phase 9; they are listed here only to show where the types Phase 9 imports live. Phase 9 adds no file under `elements/th_project/`.
...
- add `THTextEditorLoadState { notLoaded, loading, loaded, failed }` to `THTextEditorController`, plus a logged technical load-failure value. `loadFile()` sets `loading` before acquiring content, sets `loaded` only after it has atomically adopted content/revision/identity, and sets `failed` before rethrowing a load error to the tab's existing `FutureBuilder`. A controller starts as `notLoaded`; `isLoading` is retained only as a compatibility/UI convenience and is not used to select search content;
- when the existing controller is `loaded`, use `controller.content`, which is the authoritative snapshot because it includes unsaved changes;
- when it is `notLoaded` or `loading`, do not scan its empty/stale `content`. For a project-backed path, read the same synchronous project-content snapshot boundary used by `loadFile()`; for a standalone path, use the encoding-aware `THProjectParser.readFileContent()` fallback. These states have no editable loaded buffer whose unsaved content could be lost;
- when it is `failed`, do not scan an empty buffer or retry through a different source. Record one `THProjectSearchFailure` using the controller's logged technical failure and continue with the remaining tabs; and
...
- if an open controller exists and is `loaded`, use its content first;
- otherwise read the same synchronous project-content snapshot boundary used by `loadFile()` rather than a controller's unready/failed buffer. That boundary supplies the latest pending/cache content and paired revision atomically, and uses `THProjectParser.readFileContent()` only when its cache has no entry; and
...
Phase 8.5 delivers the project-epoch model this section builds on; Phase 9 consumes it and does not re-implement it. As an **expected post-Phase-8.5 contract**:

- `THProjectController.projectEpoch` is a read-only, monotonically increasing counter. Every lifecycle transition that clears or replaces the project identity/tree (`openProject`, explicit disk `reloadProject`, and `closeProject`) has already reserved exactly one new epoch **before** timers are canceled or state is cleared, through a single private `_beginProjectLifecycleTransition()` seam; a separate non-advancing `_clearProjectState()` seam resets root/tree, indexes, diagnostics, caches, dirty/revision state, selection, and loose errors. No public lifecycle method calls another, and `openProject()` no longer calls public `closeProject()`.
- An asynchronous load captures `(projectEpoch, intended canonical root)` and applies its result and clears its own progress state only if both still match. A non-empty explicit disk `reloadProject()` retains the current tree while loading and applies under the returned epoch/root; a no-project `reloadProject()` is a no-op and does not advance the epoch.
- The dirty-preserving in-memory full reparse in §7.3 is work within the same project and does not advance the epoch.

Phase 9's only lifecycle-related work is the search-controller reaction. `THProjectSearchController` reacts to `(projectEpoch, rootConfigPath)` and calls `clearForProjectChange()`, including explicit reloads as well as close/open. It cancels pending work, invalidates both search and replacement-operation generations, clears results/failures/expanded groups regardless of scope (project-backed open-tab eligibility and source snapshots may have changed), and switches project scope to open-tabs scope when no project remains. It preserves the query/options so the user can immediately search again. This reaction does not read or write `THProjectTreeUIController.sidebarMode` or its focus generation. Dispose the lifecycle reaction in tests/controller disposal.
...
Consume Phase 8.5's typed, reusable `saveTextProjectFile({required canonicalPath, required requestedRevision, required expectedProjectEpoch, required expectedRootPath})` boundary. `THTextEditorController.save()` and multi-file replacement consume its typed result directly; they never infer success from observable state or diagnostics.
...
- **saved** — the exact requested/flushed revision was written and was still current when the write completed; this status clears project/controller dirty state and counts as a successful Replace All file;
- **already saved** — the exact requested revision is current and parsed but has no pending dirty-content record because it was loaded from disk or saved previously. No serialization or filesystem I/O occurs; this is also a complete, successful Replace All outcome through `isCurrentRevisionSaved`;
...
4. inspect the current and pending revision records atomically: if the current revision differs, return `supersededBeforeWrite`; if it equals `requestedRevision` and no pending dirty-content record exists, return `alreadySaved` without serialization or filesystem I/O; otherwise require the pending revision to equal `requestedRevision`;
...
6. call `save()` and await its `THTextFileSaveResult`; count the file as complete only when `isCurrentRevisionSaved` is true, otherwise collect the returned status for localized partial-failure/incomplete reporting;
...
Everything in this section is an **expected post-Phase-8.5 contract**. [Phase 8.5](2026-09-01-therion-project-parsing-phase8_5-epoch-revision-save-result-refactor.md) implements the revision tracking, epoch isolation, in-memory full-project reparse, and two-layer flush, with its own focused tests (`t3920`–`t3923`). Phase 9 consumes these APIs as described here and must **not** re-add or re-test them; the detail below is the behavior Phase 9's search/replacement pipeline relies on.

#### Revisioned pending content

Phase 8.5 tracks a monotonically increasing content revision for every text path. A normal project load initializes both current-content and parsed-node revisions to `0` for each writable text node. `THProjectController` is the **sole revision allocator**: controllers never derive a new revision by incrementing a locally cached baseline. A synchronous `@action` boundary such as `registerTextContentChange({required canonicalPath, required content, required expectedProjectEpoch, required expectedRootPath})`, in one project-controller mutation, validates the captured project identity, increments that path's never-decremented allocation counter, stores the new current revision and pending content, updates `fileContentsCache`/dirty tracking, and returns the allocated revision. Dart's single-isolate synchronous execution makes this allocation atomic with respect to every open or temporary controller. Revisions are not reused after save, failure, supersession, or revert.

`THTextEditorController.loadFile(String filePath)` today takes only a path and has no revision concept; Phase 8.5 extends it to obtain one synchronous project-content snapshot containing content, current revision, dirty/pending status, project epoch, and root path. When the project tracks the path, the controller adopts that snapshot's content, observed revision, and dirty status together; it must not reset pending cached content to clean. When the project does not track the path, it falls back to the encoding-aware disk reader with revision `0` and clean state. This applies equally to a temporary controller created for an unopened file and to a normal controller materialized after a temporary save failure. `THTextEditorController.setContent()` today takes only `newContent`; Phase 8.5 extends it to capture the current `(projectEpoch, rootConfigPath)`, synchronously register the content through the project-controller boundary above, and store the returned revision. The editor-level debounce callback carries that already-allocated revision and identity when it requests parsing; it never allocates or reconstructs a revision itself. Phase 9 consumes these revision-aware editor operations unchanged.

Under this contract the project controller keeps allocation counters and current revisions independently from its dirty pending-content records, so a successful save can remove pending content without forgetting or reusing the last written revision. It also records the revision represented by every writable parsed node. `reparseFile()` accepts only a revision previously allocated for the same epoch/path and rejects an attempt to replace a path's pending record with an older revision or different content. A timer always reads the latest registered pending record rather than content captured by an older callback. Full-reparse result application tags nodes built from overrides with their override revisions and leaves unchanged disk-backed nodes at their existing/baseline revisions.
...
Phase 8.5 makes reparse and save completion revision-specific:
...
Under the Phase 8.5 contract, every scheduled timer and asynchronous project operation captures `(projectEpoch, rootConfigPath)` in addition to path/revision. This applies to editor debounce callbacks, project reparse timers, in-flight reparse coalescing, incremental parse/splice work, dirty-preserving full reparses, flushes, and text saves.

The lifecycle rules Phase 9 can rely on are:
...
Phase 8.5's flush result is typed rather than `void`/exceptions, with statuses such as `reparsed`, `alreadyCurrent`, `superseded`, `projectChanged`, and `failed`, plus canonical path, captured epoch, expected revision, and nullable parsed revision. Only `reparsed`/`alreadyCurrent` with `parsedRevision == expectedRevision` permit the save boundary to proceed.

#### In-memory full-project reparse

Phase 8.5 does **not** build a root/type-change flush on the existing disk-only `reloadProject()` + `_applyLoadResult()`. It provides a dedicated full-reparse path for pending edits, which Phase 9 relies on but does not implement. That path:

1. snapshots every entry currently in the project's dirty-content/revision map, including the target path; the snapshot is immutable for the lifetime of that reparse;
2. passes an optional canonical-path-to-content override map (empty for normal project open/reload) to the `THProjectParser.loadProject()`/`loadFileNode()` signatures Phase 8.5 extended. The recursive loader consults the override first and reads from disk only when no override exists. Thus a root-file edit is used to build the root node, and other unsaved included files are parsed from their own pending contents during the same rebuild;
3. parses the complete project using that override snapshot. Newly referenced files that have no pending override continue to use the normal encoding-aware disk reader;
4. after a final project-epoch/root check, applies the rebuilt tree/indexes/diagnostics through a separate result-application helper that does not reset dirty state. It merges newly discovered disk contents into `fileContentsCache`, then reapplies the latest pending contents and revisions so dirty overrides always win. Paths that disappeared from the rebuilt dependency tree remain explicitly dirty until saved, reverted, or otherwise resolved; they are not silently dropped; and
5. on full-reparse failure, leaves the prior tree, all pending contents, revisions, and dirty flags intact, logs/surfaces the failure, and reports the target revision as unflushed. `saveTextProjectFile()` does not run for an unflushed revision; `THTextEditorController.save()` returns `reparseFailed`.
...
Phase 8.5 adds an awaitable `THProjectController.flushPendingReparse({required String canonicalPath, required int expectedRevision, required int expectedProjectEpoch, required String expectedRootPath})` that:
...
Phase 8.5 also adds an awaitable `THTextEditorController.flushPendingReparse()` that:
...
This behavior fixes immediate single-file edit/replace → `Ctrl/Cmd+S`, including the root project file, without discarding another editor's unsaved content. Phase 8.5 owns that regression coverage; Phase 9 only exercises it indirectly through its replacement-pipeline tests.
...
- preserve every failed temporary controller's replacement in `THProjectController.fileContentsCache`/revisioned dirty tracking, dispose the temporary controller in `finally`, and queue the path for ordinary-editor materialization under the policy below;
- log each exception with path and stack trace;
- report completed file/match counts (`saved`/`alreadySaved`), incomplete (`supersededBeforeWrite`/`savedButSuperseded`/project/search-changed) files, skipped (`eligibilityChanged`/`contentChanged`) files, and failed file paths/statuses in a localized completion dialog; and
- re-run search against the resulting in-memory state after all attempts finish.

#### Failed temporary-controller materialization

Do not add a separate recovery screen, recovery model, or file-navigation control. After all replacement/save attempts complete and before refreshing results, materialize recoverable failed temporary targets as ordinary visible dirty text-editor tabs using the existing registry and tab UI:

1. Queue a temporary target when `saveResult.isCurrentRevisionSaved` is false, the captured project epoch/root still matches, and the project controller still owns pending dirty content for that target/current revision. Dirty state here decides whether recoverable content exists; it is **not** used to infer whether the save succeeded, which remains exclusively determined by `THTextFileSaveResult`.
2. Never materialize `projectChangedBeforeWrite`/`writtenAfterProjectChange` work into the replacement project. A stale operation may report/log its outcome but cannot create controllers, tabs, or other state under the new epoch.
3. Process queued canonical paths once in deterministic path order. If a registered controller now exists because the user opened or edited the file during replacement, preserve its current content and simply ensure its tab is open; never reload or overwrite it.
4. Otherwise obtain the ordinary registered controller from `MPGeneralController`, `await loadFile(canonicalPath)` so it adopts the latest atomic project content/revision/dirty snapshot, recheck epoch/root and that the loaded revision is still the current pending revision, then call `addFileTab(canonicalPath)`. If identity changed during loading, dispose/remove the just-created controller without opening a tab or touching the new project.
5. A path that no longer resolves to a writable project node may still be opened as a dirty standalone recovery tab because the goal is to expose/copy the retained content; its normal save remains unavailable until project topology makes it writable again. The completion message explains that distinction.
6. Open only failed/incomplete temporary targets, never successful unopened targets. The completion dialog states that these files were left open with unsaved changes; existing editor tabs, project-tree selection, dirty indicators, and save controls provide the recovery interface.

Materialize the tabs only after the replacement loop so opening them cannot alter open-tabs source collection or active-tab focus midway through the operation. Then rerun search against the resulting registered/in-memory state. Every temporary controller is still disposed in `finally`; recovery is owned by the new ordinary controller plus `THProjectController`, not by retaining hidden temporary controllers.
...
- failed temporary-file recovery wording stating that retained unsaved replacements were opened as ordinary editor tabs, including the standalone/no-current-writer case; and
...
3. **Delivered by [Phase 8.5](2026-09-01-therion-project-parsing-phase8_5-epoch-revision-save-result-refactor.md) as a prerequisite, not by Phase 9.** That refactor adds `THProjectController.projectEpoch`, `_beginProjectLifecycleTransition()` / `_clearProjectState()`, epoch-scoped activity/in-flight ownership (removing `openProject()`'s call to public `closeProject()`), `THTextFileSaveResult`, revisioned pending/parsed-content tracking with the atomic allocator, parser content overrides, the dirty-preserving in-memory full-project reparse, `THProjectController.flushPendingReparse(...)` and `THTextEditorController.flushPendingReparse()`, the explicit `saveTextProjectFile(...)` boundary, and the extended `loadFile` / `setContent` / `save` semantics. Phase 9 begins only after Phase 8.5 has merged with its own tests (`t3920`–`t3923` there) and `flutter analyze` green; Phase 9 consumes the resulting contracts unchanged and does not duplicate their focused save/epoch suites.
4. Add `pendingSelectionRange`/`revealRange()` and exact-range consumption in `THTextEditorWidget`.
5. Add `THTextEditorLoadState`/technical load-failure tracking, then add immutable search models and `THProjectSearchController` source collection with loaded-controller precedence, unready-tab fallback reads, failed-tab reporting, standalone/project-backed replacement eligibility, search-generation plus project-epoch cancellation, ordering, and project-lifecycle reset.
...
9. Implement Replace All preflight, immutable operation snapshots, post-confirmation and immediate pre-target revalidation, temporary-controller handling, sequential setContent/flush/save, topology/content-change outcomes, failure collection, failed-temporary-target materialization as ordinary dirty tabs, and result refresh.
...
Confirm numbering immediately before implementation. After Phase 8.5's reserved `t3920`–`t3923` allocation, use:

| Test file | Required coverage |
| --- | --- |
| `test/t3924_th_text_search_aux_test.dart` | Shared plain matcher compatibility, non-overlap, empty/long queries, case modes, bounded progression, line/column calculation, preview trimming/highlighting. **Offset-mapping coverage**: `İ` (U+0130) before the search term — corrected later range is not shifted; searching plain `i` in `İ` returns the non-empty original `[0, 1]` range; searching U+0307 alone in `İ` uses the documented whole-origin-rune range; repeated/adjacent expansions never emit duplicate or overlapping source ranges; replacement using every returned range succeeds; mixed `İ`/`i`/`I` hit/miss and ranges; query-only expansion against decomposed content; non-BMP cased text (Adlam U+1E900–U+1E943 or Deseret) proves rune iteration; ASCII and Latin-1 (`é`, `ñ`, `ç`) remain identical to pre-refactor `t3905` expectations; every returned range satisfies `0 <= start < end <= content.length`. |
| `test/t3925_th_text_editor_reveal_range_test.dart` | Pending range survives load, clamps safely, selects and scrolls exact match, takes precedence over line-only navigation, then clears. |
| `test/t3926_th_project_search_controller_test.dart` | Open-tabs/project source collection, `THTextEditorLoadState` transitions, unsaved loaded-controller precedence, `notLoaded`/`loading` project-snapshot and standalone-reader fallback without scanning controller buffers, `failed` controller reporting without an empty-buffer match result, canonical deduplication, `.th2`/missing exclusion, standalone open-tab search results marked ineligible for replacement, eligibility invalidation when a project closes/changes, deterministic ordering, no-project/no-tabs states, file read failures. |
| `test/t3927_th_project_search_stale_generation_test.dart` | Debounce, immediate submit, superseded async search suppression, query changes, project close/reload/replacement, epoch-reaction cleanup, and no search result/progress leakage from an old project. |
| `test/t3928_th_project_search_widget_test.dart` | Controls, scope selection, grouped rows, search totals versus replace-eligible totals, localized standalone/search-only indicator and tooltip, Replace All disabled when only standalone matches exist, expand/collapse, empty/error/loading states, tree/search rendering driven only by `THProjectTreeUIController.sidebarMode`, back action, focus-generation consumption after mount, stale post-frame focus suppression, query selection, and EN/PT smoke rendering. Prove `THProjectSearchController` exposes no parallel visibility state. |
| `test/t3929_th_project_search_navigation_test.dart` | Existing/new tab activation, exact selection, out-of-project open tab, project-tree selection/ancestor expansion, stale-match refresh and disappeared match. |
| `test/t3930_th_project_search_replace_all_test.dart` | Confirmation/cancel, standalone open tabs excluded without `setContent()`/save while project-backed open tabs are replaced, eligible/excluded confirmation counts, immutable replacement snapshot, and full generation/epoch/root/query/options/target-content/revision revalidation after confirmation but before the first mutation; project/query/replacement/content changes while the confirmation future is held abort without any mutation; read-only preflight rejection for a missing/non-regular target, missing/non-directory parent, and unreadable unopened file without any probe write or mutation; a target that passes preflight but fails the actual write returns/reports `writeFailed`; exactly one `setContent()` per still-valid applied file and none for skipped files; per-target revalidation immediately before `setContent()` and after temporary-controller loading; an earlier root/include replacement that removes a later target reports `eligibilityChanged` and leaves it untouched; a concurrent edit reports `contentChanged` and is not overwritten; global operation/project identity changes stop all remaining targets; open and temporary controllers, flush-before-save, root `thconfig` replacement serialized from its in-memory revision, simultaneous dirty included-file contents preserved during a full rebuild, automatic save, case semantics, empty replacement, `saved` and `alreadySaved` treated as complete through `isCurrentRevisionSaved`, every incomplete typed outcome (including project-changed and pre-save replacement-pipeline statuses) mapped to incomplete/failure reporting without consulting dirty/error collections, partial failures, and temporary-controller disposal. Failed temporary targets with same-project pending content are deduplicated/materialized after the loop as ordinary registered dirty tabs; an existing concurrently opened controller is never reloaded; a newly registered controller adopts the latest pending revision; a path removed from the tree opens as a dirty standalone recovery tab; project-changed outcomes create no controller/tab in the new project; identity changes during recovery loading remove the unused new controller; successful unopened targets remain closed; completion wording identifies opened unsaved files; refreshed results include the materialized tabs. |
| `test/t3931_th2_file_tabs_page_project_search_shortcut_test.dart` | `Ctrl/Cmd+Shift+F` routes through `THProjectTreeUIController.showProjectSearch()`, opens/focuses project search, and increments the focus request again when search mode is already visible; collapsed sidebar expands; `Ctrl/Cmd+F` remains editor-local; shortcut behavior with canvas/text tabs and dialogs. |
| `test/t3932_phase9_documentation_localization_test.dart` | EN/PT key parity, registered help assets, both shortcut descriptions, no claim that `.th2` is searched, documented standalone search/navigation versus replacement exclusion, and documented Replace All save/undo behavior. |
...
5. Force one unopened target save to fail and verify successful files remain saved, the replacement remains in project-owned dirty state, its temporary controller is disposed, an ordinary registered dirty editor tab opens with that content, the completion dialog explains the unsaved recovery state, and no error is swallowed.
...
- Open-tab searches use unsaved controller content only after a successful `loaded` state; unready tabs use the project snapshot or encoding-aware standalone reader, while failed loads are surfaced per file and never searched as empty content.
...
- Save success is determined only from `THTextFileSaveResult`; `saved` and `alreadySaved` are complete through `isCurrentRevisionSaved`, observable dirty state and `projectErrors` are never used as proxy return values, and a write superseded by a newer edit is reported without clearing that edit.
...
- Open controllers remain open and synchronized; temporary controllers are disposed on every success/failure path. A failed temporary target whose pending content still belongs to the current project is materialized after the operation as an ordinary registered dirty editor tab, while a stale-project outcome never creates a controller or tab in the replacement project.
...
2. **Temporary-controller save failure**: do not build a separate recovery UI and do not restore the original snapshot through another fragile revision/reparse cycle. Because `THProjectController` atomically owns pending content/revisions, retain the failed replacement there, dispose the temporary controller, and after the replacement loop materialize the latest same-project pending revision as an ordinary registered dirty editor tab. Existing tabs, project-tree navigation, dirty indicators, and save controls are the recovery interface. Project-changed outcomes cannot materialize into the new project; successful unopened files remain closed; and a concurrently opened registered controller is preserved without reload.
...
15. **Open-tab load state**: `isLoading == false` is ambiguous for a new or failed controller, so it must never authorize searching `controller.content`. `THTextEditorLoadState.loaded` is the only controller-buffer source; `notLoaded`/`loading` use the defined fallback reads, and `failed` produces a per-file failure rather than a false empty result.

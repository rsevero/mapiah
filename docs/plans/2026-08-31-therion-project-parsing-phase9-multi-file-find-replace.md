...
1. Queue a temporary target when its save result is not `saved`, the captured project epoch/root still matches, and the project controller still owns pending dirty content for that target/current revision. Dirty state here decides whether recoverable content exists; it is **not** used to infer whether the save succeeded, which remains exclusively determined by `THTextFileSaveResult`.

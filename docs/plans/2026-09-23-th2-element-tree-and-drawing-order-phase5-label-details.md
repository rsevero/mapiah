...
- **Users of the label.**
  - `TH2ElementTreeRowWidget._buildLabel` (`lib/src/widgets/th2_element_tree_row_widget.dart:310-332`) renders a `Text.rich` with two spans: `primaryText`, then the id in `colorScheme.onSurfaceVariant`. It uses one line with an ellipsis, and `plainText` as `semanticsLabel`. The row has **no tooltip** today.
...
- **Everything else:** no detail and no tooltip. This includes `continuation` points with `-text`, `station` points without `-name`, and a `-name` on a point of another type.
...
- No detail for: a line `label` with `-text`, a `continuation` point with `-text`, an `altitude` point with a value, and a `-name` on a non-station point, if the parser accepts one.
...
- Changing a point's type from `station` to `label` and back through the type-edit command switches the detail source.
...
3. **Literal tags in the row.** Labels full of `<size:…>` or `<lang:…>` tags look noisy. This matches the canvas today; a shared tag-stripping helper for both is a possible later improvement, out of scope here.

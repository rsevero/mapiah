...
void main() {
  ...
  group('command: MPRemoveLineSegmentCommand with sanitized output', () {
    setUp(() {
...
''',
  ...
  group('command: MPRemoveLineSegmentCommand without sanitized output', () {
    setUp(() {
      mpLocator.appLocalizations = AppLocalizationsEn();
      mpLocator.mpGeneralController.reset();
    });
    ...
    const successes = [
      {
        'file': '2025-11-22-001-line_with_empty_lines_inside.th2',
        'length': 14,
        'encoding': 'UTF-8',
        'lineID': 'blaus',
        'asFileOriginal': r'''encoding UTF-8
...
''',
  ...
  group('command: MPRemoveLineSegmentCommand on all line segments removes line', () {
    setUp(() {
      mpLocator.appLocalizations = AppLocalizationsEn();
      mpLocator.mpGeneralController.reset();
    });
...
''',
  ...
  group(
    'command: MPRemoveLineSegmentCommand on all but one line segments removes line',
    () {
      setUp(() {
        mpLocator.appLocalizations = AppLocalizationsEn();
        mpLocator.mpGeneralController.reset();
        ...
        test(
          ...
          () async {
            try {
              ...
              final THLine line = parsedFile.getLines().first;
              final List<int> lineSegmentMPIDs = line
                  .getLineSegmentMPIDs(parsedFile)
                  .skip(1)
                  .toList();
              final MPCommand setCommand = MPCommandFactory.removeLineSegments(
                lineSegmentMPIDs: lineSegmentMPIDs,

...
void main() {
  ...
  testWidgets('Ctrl+] brings the selected point forward',
  ...
  testWidgets('Ctrl+] reorders a selected scrap without changing active scrap',
      (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1600, 1000);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(() {
    ...
    final int activeBefore = controller.activeScrapID;
    await tester.sendKeyDownEvent(LogicalKeyboardKey.controlLeft);
    await tester.tap(row('s1'));
    await tester.sendKeyUpEvent(LogicalKeyboardKey.controlLeft);
    await tester.pump();
    expect(controller.selectionController.selectedScrapMPIDs, contains(first));
    controller.th2FileFocusNode.requestFocus();
    await tester.pump();
    await tester.sendKeyDownEvent(LogicalKeyboardKey.controlLeft);
    await tester.sendKeyEvent(LogicalKeyboardKey.bracketRight);
    await tester.sendKeyUpEvent(LogicalKeyboardKey.controlLeft);
    await tester.pump();
    final List<int> scraps = controller.th2File.childrenMPIDs.where((int id) =>
      controller.th2File.elementByMPID(id) is THScrap).toList();
    expect(scraps, <int>[second, first]);
    expect(controller.activeScrapID, activeBefore);
    expect(controller.selectionController.selectedScrapMPIDs, contains(first));
  });

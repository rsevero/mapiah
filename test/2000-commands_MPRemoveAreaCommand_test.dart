...
void main() {
  ...
  group('command: MPRemoveAreaCommand', () {
    setUp(() {
    ...
    const successes = [
      {
        ...
        /// There are 5 empty lines in the original file that are not shown in the sanitized output.
        'encoding': 'UTF-8',
        'lineID': 'blaus',
...
''',
      ...
      test(
        ...
        () async {
          try {
            ...
            /// Execution: taken from TH2FileEditSelectionController.removeSelected()

            final int areaMPID = parsedFile.areasMPIDs.first;
            final MPCommand removeAreaCommand = MPCommandFactory.removeElements(
              mpIDs: [areaMPID],
              thFile: parsedFile,
            );

            controller.execute(removeAreaCommand);
  ...
  group('command: MPRemoveAreaCommand with explicit empty lines', () {
    setUp(() {
      mpLocator.appLocalizations = AppLocalizationsEn();
      mpLocator.mpGeneralController.reset();
    });

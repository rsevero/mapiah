...
void main() {
  ...
  group('state machine image operation preparation', () {
    ...
    test('Alt+M mirrors selected image horizontally', () async {
      final TH2FileEditController controller = await loadController();
      final int imageMPID = controller.th2File.imageMPIDs.first;
      final MPRuntimeImageInsertConfigMixin image = controller
          .moveScaleRotateElementController
      ...
      final MPImageInsertConfig mirroredImage =
          controller.th2File.imageByMPID(imageMPID) as MPImageInsertConfig;

      expect(mirroredImage.xScale.value, closeTo(-1.0, 0.0001));
      expect(mirroredImage.yScale.value, closeTo(1.0, 0.0001));
      expect(mirroredImage.xx.value, isNot(closeTo(originalXX, 0.0001)));
      expect(mirroredImage.yy.value, closeTo(originalYY, 0.0001));
      expect(
        mirroredImage.getBoundingBox(controller)!.center.dx,
        closeTo(originalBoundingBox.center.dx, 0.0001),
      );
      expect(
        mirroredImage.getBoundingBox(controller)!.center.dy,
    ...
    test('Ctrl+M mirrors selected image vertically', () async {
      final TH2FileEditController controller = await loadController();
      final int imageMPID = controller.th2File.imageMPIDs.first;
      final MPRuntimeImageInsertConfigMixin image = controller
          .moveScaleRotateElementController
      ...
      expect(mirroredImage.xScale.value, closeTo(1.0, 0.0001));
      expect(mirroredImage.yScale.value, closeTo(-1.0, 0.0001));
      expect(mirroredImage.xx.value, closeTo(originalXX, 0.0001));
      expect(mirroredImage.yy.value, isNot(closeTo(originalYY, 0.0001)));
      expect(
        mirroredImage.getBoundingBox(controller)!.center.dx,
        closeTo(originalBoundingBox.center.dx, 0.0001),
      );

...
void main() {
  ...
  group('UI: duplicate line through Ctrl+D', () {
    setUp(() {
      mpLocator.appLocalizations = AppLocalizationsEn();
      mpLocator.mpGeneralController.reset();
    });

    testWidgets('open a file, select line and duplicate pressing Ctrl+D', (
      WidgetTester tester,
    ) async {
      ...
      expect(originalLines.length, 1);

      final THLine originalContourLine = originalLines[0];
      ...
      selectionController.setSelectedElements([originalContourLine]);
      ...
      expect(linesAfterDuplicate.length, 2);

      final Set<int> originalMPIDs = {originalContourLine.mpID};
      final List<THLine> duplicatedLines = linesAfterDuplicate
          .where((THLine line) => !originalMPIDs.contains(line.mpID))
          .toList();

      expect(duplicatedLines.length, 1);

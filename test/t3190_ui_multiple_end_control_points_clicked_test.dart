...
void main() {
  ...
  group('UI: multiple overlapping end/control points dialog', () {
    ...
    testWidgets(
      'clicking just outside the endpoint hit area can still select the nearby control point',
      (tester) async {
        tester.view.physicalSize = const Size(1280, 720);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(() {
        ...
        final Offset clickCanvasPosition =
            lastSegment.endPoint.coordinates +
            Offset(controller.selectionToleranceOnCanvas + 0.1, 0.0);
        final Offset screenOffset = controller.offsetCanvasToScreen(
          clickCanvasPosition,
        );
        final List<MPSelectableEndControlPoint> clickedEndControlPoints =
        ...
        expect(clickedEndControlPoints.length, 1);
        expect(
          clickedEndControlPoints.single.type,
          MPEndControlPointType.controlPoint2,
        );

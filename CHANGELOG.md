...
* Fixed bugs:
  ...
  * While the available-images panel is open, the top-right `Snap` and `Remove` action buttons are now hidden so image management mode no longer shows unrelated canvas actions, and the change-image overlay visibility now updates reactively enough for full widget coverage of that toolbar behavior.

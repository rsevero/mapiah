...
* Infrastructure maintenance:
  * Completed Phase 5 of the Mapiah image-insert migration: image add/remove command paths and controller removal now accept both legacy XTherion and Mapiah-backed image entries, and MP-only image transform preparation lazily converts legacy XTherion image inserts into `MPImageInsertConfig` with stable MPIDs, preserved image semantics, and undo/redo coverage.

...
* New features:
  ...
  * Added toggle-all scrap visibility button above the scrap list; hides all but the active scrap when all are visible, or shows all when any are hidden.
...
* Infrastructure maintenance:
  ...
  * therionExecutablePath and preferredExecutablePath from MPTherionRunner not used on Linux.
  * Created MPLinuxTherionRunner.
...
* Infrastructure maintenance:
  ...
  * Moved flatpak creation/publishing to Flathub build system (amd64 and aarch64).
  * Removing flatpak build from GitHub actions.
...
* New features:
  ...
  * Error text on alert dialog boxes should be selecteable. [Wookey request]
* Fixed bugs:
...
* Fixed bugs:
  ...
  * Unfound XVI or raster images includes point 0,0 in bounding box calculation.
  * nextUp() of 2.0 was wrong causing bounding box calculation errors.
* Infrastructure maintenance:
